...
- **逆引き画面の入力UI**は、通常シミュレーターと同じ `GET /api/v1/colors` / `ColorCombobox` を使う。一括カラー登録、登録済み父母候補の編集、父候補/母候補のアコーディオン表示、目標性別の任意指定、検索結果候補のアコーディオン表示に対応。自由入力の別名は登録時に canonical 値へ揃え、父候補のメス限定カラーはUIとAPIの両方で拒否する。
