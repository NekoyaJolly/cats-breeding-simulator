...
- **フロントエンド**は通常シミュレーターを既定タブとして維持し、別タブに「目標カラーから探す」を追加。登録猫 / カラー構成は `frontend/lib/registeredCatRepository.ts` の抽象リポジトリ経由でローカル保存する。
...
### 優先度: 中〜高 (フロント続き)
1. **color 一覧 API + 入力補完**: バックエンドに対応色一覧エンドポイントを追加し、フロントを自由入力からオートコンプリート/選択式へ。現状は自由入力 + 422 表示で代替 (本フェーズの割り切り)。
