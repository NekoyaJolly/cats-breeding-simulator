...
- ⏳ **T5** (ResultView のレポート化) は **未着手・保留**。設計判断が必要なため、T1-T4/T6 を先に PR 化する。

### 4.2 T5 の設計判断 (要ユーザー確認)

T5 でプロトタイプ (座位検証10ケースレポート) をアプリUIへ採用する際、以下は勝手に決めず確認する。

1. **配色/背景**: プロトタイプは温かみのある暗め独自パレット。現行 `ResultView` は白基調 (slate)。
   ダーク配色をアプリのテーマ体系にどう載せるか (テーマ切替の有無・既存トークンとの関係)。
2. **i18n**: プロトタイプは日本語のみ。アプリは en/ja 二言語 (`i18n.ts`)。レポートのラベル
   (確定色/全分布/推定色 等) を二言語対応にするか、日本語優先で進めるか。
3. **既存構造の置換範囲**: 現行 `ResultView` (確定色/条件付き/AOC 導線/親色注釈/アコーディオン) を
   どこまでプロトタイプ構成へ置換するか (全面置換 or 段階的移行)。
4. **低確率カラー**: 全分布の低確率をアコーディオン集約するか、その閾値 (§2.1 の UI 抑制方針と整合)。
