...
## 6. 今後の細部調整 (フェーズ完了後の任意課題)

本フェーズの設計判断 (配色=独自ダークトークン / 低確率=1%未満アコーディオン集約 / en-ja 二言語 /
`ResultView` 全面置換) は §4.2 で確定・実装済み。以下は完了に必須ではない任意の磨き込み。

- レポートの余白・階調・タイポグラフィの微調整 (実機フィードバックに応じて)。
- carrier_exploration 撤去 (#52) の API 破壊的変更は内部アプリのため外部バージョニング告知は不要と判断。
  外部連携が生じた場合のみ再検討。
